...
import { Heart, User, Menu, LayoutDashboard, X, Bookmark } from 'lucide-react';
    ...
    <header className="sticky top-0 z-50 w-full border-b border-slate-200 bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/60 no-print">
      <div className="container mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <Link to="/" className="flex items-center space-x-2 flex-shrink-0">
          <span className="text-xl md:text-2xl font-heading font-bold text-primary truncate">emmanuelhl0</span>
        </Link>

        {/* Desktop Navigation */}
        <nav className="hidden lg:flex items-center space-x-6 text-sm font-medium">
          <Link to="/" className="text-secondary-dark hover:text-primary transition-colors">Accueil</Link>
          <Link to="/a-propos" className="text-secondary-dark hover:text-primary transition-colors">À À Propos</Link>
        ...
        <div className="flex items-center space-x-2 md:space-x-4">
          ...
          {!isAdmin && (
            ...
            >
              <Heart className="h-5 w-5 md:mr-2" />
        ...
        {isMobileMenuOpen && (
          ...
          >
            <div className="flex flex-col p-4 space-y-4 text-sm font-medium">
              ...
              <Link to="/a-propos" onClick={() => setIsMobileMenuOpen(false)} className="p-2 hover:bg-slate-50 rounded">À À Propos</Link>
