...
const Quote = () => {
  ...
  const clientCity = "Abidjan"; // Simulation, pourrait être dynamique
  const totalItems = cart.reduce((sum, item) => sum + item.quantity, 0);
  const deliveryPricePerItem = clientCity.toLowerCase() === "abidjan" ? 3000 : 10000;
  ...
  if (cart.length === 0) {
    return (
      <div className="container mx-auto px-4 py-20 text-center">
        <h2 className="text-2xl font-bold mb-4">Votre sélection est vide pour un devis.</h2>
      ...
      <div ref={quoteRef} className="bg-white p-12 rounded-xl shadow-lg border border-slate-200 min-h-[1000px] flex flex-col">
        ...
        <div className="flex justify-between items-start border-b-2 border-primary pb-8 mb-8">
          ...
          <div className="text-right">
            <h2 className="text-xl font-bold">emmanuelhl0</h2>
            <p className="text-sm text-secondary">Abidjan, Côte d'Ivoire</p>
            <p className="text-sm text-secondary">contact@emmanuelhl0manuel.cm</p>
        ...
        <div className="mt-20 text-[10px] text-secondary text-center space-y-1">
          <p>emmanuelhl0 - RCCM : CI-ABJ-03-202X-B-XXXXX - NID : XXXXXXXXXX</p>
