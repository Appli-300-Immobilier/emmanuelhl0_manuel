...
import { ArrowRight, Truck, ShieldCheck, Clock, ChevronRight } from 'lucide-react';
...
const Home = () => {
  ...
  const slides = [
    ...
    },
    {
      ...
      subtitle: `emmanuelhl0 accompagne le développement industriel et domestique avec des solutions innovantes et durables.`,
  ...
  return (
    <div className="flex flex-col gap-24 pb-20 overflow-x-hidden relative">
      ...
      {/* Hero Section */}
      <section className="min-h-[95vh] flex items-center relative overflow-hidden text-white">
        {/* Background Image with Luxury Overlay */}
        <div className="absolute inset-0 z-0">
          <img
            src="https://loremflickr.com/1920/1080/luxury,architecture,luxury,architecture?lock=hero"
        ...
        <div className="container mx-auto px-4 relative z-10 py-12 md:py-20">
            ...
            >
              ...
              <div className="flex-1 space-y-6 md:space-y-10 text-center lg:text-left order-2 lg:order-1">
                ...
                >
                  ...
                  <motion.p
                    variants={itemVariants}
                    className="text-base sm:text-lg md:text-xl lg:text-2xl text-primary-light max-w-2xl mx-auto lg:mx-0 leading-relaxed font-light opacity-90"
                  >
              ...
              <div className="flex-1 w-full max-w-[280px] sm:max-w-md lg:max-w-xl perspective-1000 order-1 lg:order-2">
                <TiltWrapper className="glass-card p-6 sm:p-12 rounded-[2rem] sm:rounded-[3rem] relative overflow-hidden group">
                  <motion.div className="absolute inset-0 bg-gradient-to-tr from-white/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700" />
                  <div className="relative z-10 text-center space-y-4 sm:space-y-8">
                    ...
                    <div className="space-y-1 sm:space-y-2">
                      <h3 className="text-xl sm:text-3xl font-heading font-bold tracking-tight">
                        {slides[currentSlide].iconLabel}
                      </h3>
          ...
          <div className="flex justify-center lg:justify-start gap-4 mt-16">
            {slides.map((_, idx) => (
              <button key={idx} onClick={() => setCurrentSlide(idx)} className="group p-2">
                <div
      ...
      <AnimatedSection className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <FeatureCard
            icon={Truck}
            title="Visite Guidée"
            description="Découvrez vos futurs biens avec nos conseillers experts directement sur le terrain."
          />
          <FeatureCard
            icon={ShieldCheck}
            title="Transactions Garanties"
            description="Profitez d'un cadre juridique sécurisé pour toutes vos transactions immobilières."
          />
          <FeatureCard
            icon={Clock}
            title="Accompagnement VIP"
            description="Un conseiller personnel dédié vous accompagne à chaque étape de votre acquisition."
          />
        </div>
      </AnimatedSection>
      ...
      <AnimatedSection className="container mx-auto px-4">
        <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-16">
          <div className="space-y-4">
            <h2 className="text-5xl font-heading font-bold text-secondary-dark tracking-tighter">
              Sélection Vedette
            </h2>
            <p className="text-secondary text-lg font-light">
              Les meilleures opportunités immobilières sélectionnées pour vous.
            </p>
          </div>
      ...
      <div ref={sectorsRef}>
        <AnimatedSection className="container mx-auto px-4 bg-slate-50/50 py-16 md:py-24 rounded-[3rem] md:rounded-[4rem] border border-slate-100">
          <div className="text-center max-w-3xl mx-auto mb-16 md:mb-20 space-y-4">
            <h2 className="text-3xl md:text-5xl font-heading font-bold text-secondary-dark tracking-tighter uppercase">
              Secteurs d&apos;Activité
            </h2>
            <div className="h-1 w-16 md:h-1.5 md:w-20 bg-primary mx-auto rounded-full" />
            <p className="text-secondary text-lg md:text-xl font-light pt-4 px-4">
              L&apos;expertise votre partenaire de confiance au service de votre performance.
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {[
              { title: 'Vente de Terrains', icon: '🏗️', color: 'from-orange-400 to-red-500' },
              { title: "Location d'Appartements", icon: '🏢', color: 'from-blue-400 to-indigo-600' },
              { title: 'Villas de Luxe', icon: '🏡', color: 'from-cyan-400 to-blue-500' },
              { title: 'Sécurité (Vidéosurveillance & Alarme)', icon: '🔒', color: 'from-red-400 to-rose-600' },
              { title: 'Conseil en Investissement', icon: '📈', color: 'from-purple-400 to-violet-600' },
              { title: 'Architecture & Design', icon: '📐', color: 'from-yellow-400 to-orange-500' },
              { title: 'Rénovation & BTP', icon: '🛠️', color: 'from-sky-400 to-cyan-600' },
              { title: 'Expertise Foncière', icon: '📜', color: 'from-pink-400 to-rose-500' },
              { title: 'Promotion Immobilière', icon: '🏗️', color: 'from-indigo-400 to-blue-700' },
              { title: 'Locations de Vacances', icon: '🏖️', color: 'from-emerald-400 to-teal-600' },
            ].map((cat, idx) => (
      ...
      <AnimatedSection className="container mx-auto px-4 mb-20">
        <motion.div
          className="relative bg-secondary-dark rounded-[3rem] md:rounded-[4rem] p-10 md:p-32 overflow-hidden text-center shadow-[0 50px 100px -20px rgba(0,0,0,0.5)]"
        ...
        >
          ...
          <div className="relative z-10 space-y-8 md:space-y-10">
            <h2 className="text-3xl md:text-7xl font-heading font-bold text-white tracking-tighter leading-tight">
              Un projet <span className="text-primary italic">sur-mesure</span> ?
            </h2>
            <p className="text-slate-400 text-lg md:text-2xl max-w-2xl mx-auto font-light leading-relaxed">
              De l&apos;acquisition de terrain à la remise des clés, nous bâtissons votre patrimoine.
            </p>
            <div className="flex justify-center pt-6 md:pt-10">
              <motion.div
                animate={{
                  boxShadow: [
                    '0 0 0px rgba(0,86,179,0)',
...
const FeatureCard = ({ icon: Icon, title, description }) => (
  <motion.div
    ...
    className="bg-white p-12 rounded-[3rem] border border-slate-100 shadow-sm hover:shadow-[0 30px 60px -15px rgba(0,0,0,0.1)] transition-all duration-500 group relative overflow-hidden"
  >
...
const CategoryCard = ({ title, icon, color }) => (
  <Link to={`/catalogue?category=${title}`} className="group block">
    <motion.div
      ...
      className={`aspect-square rounded-[3rem] flex flex-col items-center justify-center gap-8 transition-all duration-500 shadow-sm hover:shadow-[0 20px 40px -10px rgba(0,0,0,0.1)] bg-gradient-to-br ${color} p-10 relative overflow-hidden`}
    >
      ...
      <span className="font-bold text-center leading-tight text-white text-lg drop-shadow-md">{title}</span>
    </motion.div>
  </Link>
);
